...
export class AppService {
    ...
    initStoreData() {
        ...
        /* val = localStorage.getItem('sidebar'); // open, closed
        val = JSON.parse(val) || 'false'; // default to closed if not set
        this.store.dispatch({ type: 'toggleSidebar', payload: val }); */
    ...
    toggleLanguage(item: any) {
        ...
        if (!lang) {
            lang = this.storeData.languageList.find((d: any) => d.code === 'ae');
        }
